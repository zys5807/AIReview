...
FUTURES_MULTIPLIERS: dict[str, tuple[str, float]] = {
    ...
    "CY": ("棉纱", 5), "PR": ("瓶片", 15), "PL": ("丙烯", 20),
    "RI": ("早籼稻", 20), "JR": ("粳稻", 20), "LR": ("晚籼稻", 20),
    # 广期所（GFEX）
    "SI": ("工业硅", 5), "LC": ("碳酸锂", 1),
    "PS": ("多晶硅", 3), "PT": ("铂", 1000), "PD": ("钯", 1000),
    # 能源中心（INE）
    "SC": ("原油", 1000), "LU": ("低硫燃油", 10),
    "NR": ("20号胶", 10), "BC": ("国际铜", 5), "EC": ("集运欧线", 50),
    # 中金所（CFFEX）——乘数为"元/点"，公式不变：占用 = 价格 × 手数 × 乘数 × 保证金率
    "IF": ("沪深300", 300), "IH": ("上证50", 300),
    "IC": ("中证500", 200), "IM": ("中证1000", 200),
    "T": ("10年期国债", 10000), "TF": ("5年期国债", 10000),
    "TL": ("30年期国债", 10000), "TS": ("2年期国债", 20000),
}
...
FUTURES_ALIASES: dict[str, list[str]] = {
    ...
    "EC": ["集运欧线", "欧线集运", "集运指数"],
    # 中金所（CFFEX）
    "IF": ["沪深300", "沪深300指数", "沪深300期货"],
    "IH": ["上证50", "上证50指数", "上证50期货"],
    "IC": ["中证500", "中证500指数", "中证500期货"],
    "IM": ["中证1000", "中证1000指数", "中证1000期货"],
    "T": ["十年国债", "10年国债", "10年期国债"],
    "TF": ["五年国债", "5年国债", "5年期国债"],
    "TL": ["三十年国债", "30年国债", "30年期国债", "超长期国债"],
    "TS": ["二年国债", "2年国债", "2年期国债"],
}
