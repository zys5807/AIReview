...
- 📦 **最新版下载**：请前往 [GitHub Releases 页面](https://github.com/zys5807/AIReview/releases/latest) 下载 `AIReviewSystem-V1.008-清洁版.zip`
...
- 📊 **交易统计**：盈亏、胜率、盈亏比、持仓周期、手续费等分维度统计，支持分时段（早盘/午盘/夜盘）分析；**手续费计入每笔盈亏（净盈亏 = 盈亏金额 − 手续费）**，收益率/胜率/期初期末资金/回撤等全部按净额口径
- 💰 **收益率与仓位**：按品种自动计算占用资金（87 个期货品种乘数表 + 东方财富每日自动同步保证金率，支持品种/合约两级参数覆盖与名称容错匹配）与单笔收益率；账户资金流水管理（支持人民币 CNY / 美元 USD 双币种，USDT 按 1:1 并入美元），交易计划自动计算单笔/总仓位比例；数字货币占用资金 = USDT 持仓规模
- 📈 **阶段绩效指标**：阶段期初/期末资金（随所选时间段动态计算、按币种分别展示，含此前交易盈亏）、平均单笔盈亏比、日平均仓位、阶段总收益率、最大回撤、最大周度回撤、卡玛比率、夏普比率（需先录入账户资金流水）
